...
namespace DesktopApp.PanelControls
{
    public partial class ProductManagerControl : UserControl
    {
        ...
        private async void btnRemoveProduct_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show($"Are you sure you want to remove {productName}?", "Confirm Removal", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

            if (result == DialogResult.Yes)
            {
                try
                {
                    await _productManager.RemoveProductByIdAsync(productId);

                    dgvProducts.Rows.Remove(selectedRow);

                    MessageBox.Show($"Product \"{productName}\" has been removed.", "Product Removed", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception ex)
                {
